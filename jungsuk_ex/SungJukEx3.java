...
class ClassTotalComparator implements Comparator<Student3> { 
	public int compare(Student3 s1, Student3 s2) { 
		...
		int result = -1 ;
//			result = s2.classNo - s1.classNo ;  // 내림차순
			result = s1.classNo - s2.classNo ;  // 오름차순
			
			if (result == 0 ) {     // 자기자신끼리 뺀것만 찾음 
//				result = s1.total - s2.total ;  // 오름차순
				result = s2.total - s1.total ;  // 내림차순
			}
...
class ClassStudentNoComparator implements Comparator<Student3> { 
	public int compare(Student3 s1, Student3 s2) { 
		...
		int result = -1 ; 
			...
			if (result == 0 ) {
//				result = s2.studentNo - s1.studentNo ; //  내림차순 
				result = s1.studentNo - s2.studentNo ; //  오름차순
			}
